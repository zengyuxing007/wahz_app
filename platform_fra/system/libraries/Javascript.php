 ...
 * @link		http://mobagepf.com/user_guide/libraries/javascript.html
 */
class MG_Javascript {
	...
	public function __construct($params = array())
	{
		...
		$this->MG->load->library('javascript/'.$js_library_driver, array('autoload' => $autoload));
		// make js to refer to current library
		$this->js =& $this->MG->$js_library_driver;
	...
	function external($external_file = '', $relative = FALSE)
	{
		...
		{
			if ($this->MG->config->item('javascript_location') != '')
			{
				$this->_javascript_location = $this->MG->config->item('javascript_location');
			}
		}
		...
		{
			$str = $this->_open_script($this->MG->config->slash_item('base_url').$this->_javascript_location.$external_file);
		}
	...
	{
		$str = '<script type="text/javascript" charset="'.strtolower($this->MG->config->item('charset')).'"';
		$str .= ($src == '') ? '>' : ' src="'.$src.'">';
		return $str;
	}
